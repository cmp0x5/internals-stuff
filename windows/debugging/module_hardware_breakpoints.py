...
def handle_event_load_dll(pEvent):
    ...
    for exp in pe.DIRECTORY_ENTRY_EXPORT.symbols:
        if exp.name == b"NtWriteFile":
            ...
            #set hardware bp at NtWriteFile
            ...
            context.Dr0 = writefile_address
            print(f"\nSet Dr0 to {context.Dr0}\n")
            context.Dr7 &= ~(0xF << 16) # clear bits 16-19 to set hardware breakpoint on execution for byte at NtWriteFile
            context.Dr7 |= 1 # set bit 0 to enable dr0
            print(f"\nSet Dr7 to {context.Dr7}\n")
            
            set_context_status = win32process.SetThreadContext(target_thread_handle, context)
            
            
            print(f"Set hardware breakpoint at {hex(writefile_address)}")
            break
    # Because we handled the event return a status of DBG_CONTINUE
    return win32types.DBG_CONTINUE

def handle_single_step(pEvent):
    global target_process_handle
    global target_path
    global entry_point_breakpoint_address
    global entry_point_breakpoint_byte
    ...
    # check if single step comes from trap flag or from hardware breakpoint
    context = win32process.GetThreadContext(target_thread_handle)
    # check if Break Status bit (14) is set, indicating single step came from Trap Flag
    if context.Dr6 & (1<<14):
        print(f"\nTRAP FLAG SET")
    elif context.Dr6 & 0xF:
    ...
    while True:
        ...
        if ctypes.windll.kernel32.WaitForDebugEvent(ctypes.byref(pEvent), 100):
            ...
            if pEvent.dwDebugEventCode == win32types.CREATE_PROCESS_DEBUG_EVENT:
                ...
                # Handle software or hardware breakpoint exception events
                if exception_code == win32types.EXCEPTION_BREAKPOINT: 
                    ...
                    dwStatus = handle_single_step(pEvent)
